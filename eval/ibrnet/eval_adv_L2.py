...
# GLOBAL Variable for saving things to folder...
save_results = True
...
if __name__ == '__main__':
            ...
            for data in train_loader:
                if args.use_unseen_views:
                    ...
                    if args.sample_based_on_depth:
                        ...
                        # Randomize rays chosen for initial perturbation...
    ...
    for i, data in enumerate(test_loader):
        ...
        if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_average.png'.format(file_id)), averaged_img)
        ...
        if args.export_adv_source_img:
            ...
            for j in range(adv_src_rgbs.shape[0]):
                ...
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, 'adv_src_{}_{}.png'.format(i,j)), adv_src_img)
                
                src_img = src_rgbs[j,:,:,:]
                src_img = (255 * np.clip(src_img.data.cpu().numpy(), a_min=0, a_max=1.)).astype(np.uint8)
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, 'src_{}_{}.png'.format(i,j)), src_img)
        ...
        with torch.no_grad():
            ...
            if args.use_clean_color or args.use_clean_density:
                print("Rendering with clean color and density...")
            ...
            if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_err_map_coarse.png'.format(file_id)), coarse_err_map_colored)
            ...
            if ret['outputs_fine'] is not None:
                ...
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_err_map_fine.png'.format(file_id)), fine_err_map_colored)

                fine_pred_rgb = (255 * np.clip(fine_pred_rgb.numpy(), a_min=0, a_max=1.)).astype(np.uint8)
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_pred_fine.png'.format(file_id)), fine_pred_rgb)
                fine_pred_depth = ret['outputs_fine']['depth'].detach().cpu()
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_depth_fine.png'.format(file_id)), (fine_pred_depth.numpy().squeeze() * 1000.).astype(np.uint16))
                
                fine_pred_depth_colored = colorize_np(fine_pred_depth,
                                                      range=tuple(data['depth_range'].squeeze().cpu().numpy()))
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_depth_vis_fine.png'.format(file_id)), (255 * fine_pred_depth_colored).astype(np.uint8))

                if 'depth' in data:
                    fine_gt_depth_colored = colorize_np(data['depth'][0],
                                                        range=tuple(data['depth_range'].squeeze().cpu().numpy()))
                    if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_depth_vis_GT.png'.format(file_id)),
                ...
                if save_results: imageio.imwrite(os.path.join(out_scene_dir, '{}_acc_map_fine.png'.format(file_id)),
